...
- **The Windows firewall prompt is a real UX problem, and it is a `#build` problem.** Today the game
  ...
  **Both are done (r44).** `Transport_Udp` binds port 0 and never asks for an inbound mapping, and
  ...
  prompt names the game. It carries the icon too since d11 → B (r50), and since r71 that icon is the
  game's logo, the canoe across the moon, in `desktop/packaging/`, on the binary and on the window.
...
has a 24 B header (27 B since d14 added the run number and the score screen flag), 6 B per player in the score table (7 B since r69 gave a row the look of the player's last hero), 21 B per hero *with* its axe, 7 B per monster
...
no layout fits — since r38 it does (`Snapshot_View` drops heroes already queued to die), and since
r63 the host destroys a potion as soon as it falls below the world (`GVars_Game.removeFallenPotions`),
so the same seed-1 run now peaks at 434 B.
...
hero already queued to die (a potion below the world is destroyed by the run itself, n7), and it always sends monsters, because
