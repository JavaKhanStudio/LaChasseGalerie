...
vps "$compose pull -q && $compose up -d --force-recreate && sleep 3 && $compose ps --format '{{.Name}} {{.State}} {{.Status}}' && sudo ss -ulpn | grep ':7770 ' && sudo ss -tlpn | grep ':7771 ' && $compose logs --tail 5 lobby"
